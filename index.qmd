...
leaflet() %>% 
  ...
  addPolygons(data = BGs,
    ...
    addPolygons(data = UBAbgs,
              ...
              popup = UBAbgpopup,
              group = "Burdened Area") %>% 
  addLayersControl(overlayGroups = c("1/4 Mile Radius", "Transmission Line", 
                                     "Census Block Groups", "Burdened Area"),
  ...
  #           colors = "red", 
  #           opacity = 0.5,
  #           labels = "Burdened Area",
  #           layerId = "Burdened Area") %>% 
  # addLegend(position = "bottomright",  
  #           colors = "black", 
  #           opacity = 0.7,
  #           labels = "Transmission Line",
  #           layerId = "Transmission Line")
```
...
@tbl-bgs shows the MassEnviroScreen indicator values for the Burdened Area within the SGA. Items in red exceed the 50th percentile relative to non-Burdened Areas of the state. 
...
BAscores <- bind_cols(indiRaw, select(indiNorm, Percentile)) %>% 
  ...
  mutate(Percentile = cell_spec(Percentile, color = case_when(
    ...
    #   nonBAflag == "Yes" ~ "red",
    #   .default = "black")),
    PercentileNBAcol = cell_spec(PercentileNBA, color = case_when(
...
#   kbl("html", escape = FALSE, 
#       caption = "MassEnviroScreen Indicators for Burdened Area within SGA", 
#       col.names = c("Indicator", "Raw Value", "Percentile Relative to non-Burdened Areas of the State")) %>% 
#   kable_styling() %>% 
#   scroll_box(width = "80%", height = "500px")

BAscores %>% 
  select(-Percentile, -PercentileNBAcol) %>% 
  datatable(., rownames = FALSE, 
            caption = "MassEnviroScreen Indicators for Burdened Area within SGA", 
            colnames = c("Indicator" = "indicator", "Raw Value" = "RawValue", 
            "Percentile Relative to non-Burdened Areas of the State" = "PercentileNBA")) %>% 
  formatStyle(columns = "Percentile Relative to non-Burdened Areas of the State", 
              color = styleInterval(c(50),c("black", "red")))
```


## Step 4. Identify Impacts on Key Indicators

Using the list of relevant, technology-specific indicators established by the EFSB, the project applicant must assess and document the anticipated impact of the proposed project on each indicator within the SGA. The project applicant must determine whether the project will exacerbate any indicator that already exceeds the 50th percentile threshold or cause any additional indicator to exceed the 50th percentile threshold. If either condition is met, the impact is considered a disproportionate adverse effect for that indicator.

@tbl-kpi below lists the key indicators by technology type as identified by the EFSB. 'X' indicates a facility’s construction and/or operation has the ability to directly impact the listed indicator.
...
font.size <- "10pt" # font size for datatable in combination with JS solution below
...
read_xlsx("data/CIA Indicators by Technology 08-01-25.xlsx", 
  ...
  datatable(., rownames = FALSE, caption = "Key Indicators by Technology Type. 'X' indicates facility’s construction and/or operation has the ability to directly impact the listed indicator.", 
...
### 4a. Determine Project Impact Level

The impact on each key indicator is assessed using a qualitative ranking of project impact, which is based on guidelines provided by OEJE and EFSB. Project impact ranks can be one of the following:
...
Project Impact Ranks will be determined for Construction phase and Operation phase separately. The project proponent will be provided with clear and concise guidance on assigning a Project Impact Rank. Guidance should be provided for evaluating impacts on each unique indicator. 


### 4b. Apply Decision Rules to Determine if Disproportionate Adverse Effect

Whether a project's impact on a key indicator rises to the level of a disproportionate adverse effect is determined based on decision rule matrix as exemplified below in @tbl-dae. 
...
data.frame(indicator = c(rep(">=75",5), rep("50-74",5), rep("40-49",5), rep("<40",5)), 
           PIF = rep(c("No Impact", "Negligible", "Minor", "Moderate", "Major"),4),
           DAE = c("No", "Yes", "Yes", "Yes", "Yes", "No", "No", "Yes", "Yes", "Yes", "No", "No", "No", "Yes", "Yes", "No", "No", "No", "No", "Yes")) %>% 
  kbl("html", escape = FALSE, 
      caption = "Decision Matrix for Project Impacts", 
      col.names = c("Indicator Percentile", "Project Impact Rank", "Disproportionate Adverse Effect?")) %>% 
  kable_styling() %>% 
  collapse_rows(columns = 1, valign = "middle")
```


In the case of the current project simulation for the Sudbury Hudson transmission line, the project's projected impacts on key indicators show that it would have a disproportionate adverse effect on several indicators (DWATER, cleanup_score, and Heatmean) as a result of the high non-Burdened Area percentile values for those indicators in combination with the project impact ranks for the construction and/or operations phases. 
...
PIFscores <- data.frame(PIF, cPIF, oPIF) %>% 
  mutate(cPIFlevel = case_when(
           cPIF < -.75 ~ "Major Benefit",
           cPIF < -.5 & cPIF >= -.75 ~ "Moderate Benefit",
           cPIF < -.25 & cPIF > -.5 ~ "Minor Benefit",
           cPIF < 0 & cPIF >= -.25 ~ "Negligible Benefit",
           cPIF == 0 ~ "No Impact",
           cPIF > 0 & cPIF <= .25 ~ "Negligible Burden",
           cPIF > .25 & cPIF <= .5 ~ "Minor Burden",
           cPIF > .5 & cPIF <= .75 ~ "Moderate Burden",
           cPIF > .75 ~ "Major Burden"
         ),
         oPIFlevel = case_when(
           oPIF < -.75 ~ "Major Benefit",
           oPIF < -.5 & oPIF >= -.75 ~ "Moderate Benefit",
           oPIF < -.25 & oPIF > -.5 ~ "Minor Benefit",
           oPIF < 0 & oPIF >= -.25 ~ "Negligible Benefit",
           oPIF == 0 ~ "No Impact",
           oPIF > 0 & oPIF <= .25 ~ "Negligible Burden",
           oPIF > .25 & oPIF <= .5 ~ "Minor Burden",
           oPIF > .5 & oPIF <= .75 ~ "Moderate Burden",
           oPIF > .75 ~ "Major Burden"
         ),
         PIFsum = cPIF+oPIF,
         PIFlevel = case_when(
           PIFsum < -.75 ~ "Major Benefit",
           PIFsum < -.5 & PIFsum >= -.75 ~ "Moderate Benefit",
           PIFsum < -.25 & PIFsum > -.5 ~ "Minor Benefit",
           PIFsum < 0 & PIFsum >= -.25 ~ "Negligible Benefit",
           PIFsum == 0 ~ "No Impact",
           PIFsum > 0 & PIFsum <= .25 ~ "Negligible Burden",
           PIFsum > .25 & PIFsum <= .5 ~ "Minor Burden",
           PIFsum > .5 & PIFsum <= .75 ~ "Moderate Burden",
           PIFsum > .75 ~ "Major Burden"
         ))

# bring in BAscores and append PIF
BAscores %>% 
  inner_join(., PIFscores, by = c("indicator" = "PIF")) %>% 
  mutate(DAE = case_when(
      PercentileNBA >= 75 & (cPIFlevel %in% c("Negligible Burden", "Minor Burden", "Moderate Burden", "Major Burden") | oPIFlevel %in% c("Negligible Burden", "Minor Burden", "Moderate Burden", "Major Burden")) ~ "Yes",
      PercentileNBA >= 50 & PercentileNBA <= 74 & (cPIFlevel %in% c("Minor Burden", "Moderate Burden", "Major Burden") | oPIFlevel %in% c("Minor Burden", "Moderate Burden", "Major Burden")) ~ "Yes",
      PercentileNBA >= 40 & PercentileNBA <= 49 & (cPIFlevel %in% c("Moderate Burden", "Major Burden") | oPIFlevel %in% c("Moderate Burden", "Major Burden")) ~ "Yes",
      PercentileNBA < 40 & (cPIFlevel == "Major Burden" | oPIFlevel == "Major Burden") ~ "Yes",
      .default = "No"),
      DAE = cell_spec(DAE, color = case_when(
    DAE == "Yes" ~ "red",
    .default = "black"))
    ) %>% 
  select(indicator, RawValue, PercentileNBAcol, cPIFlevel, oPIFlevel, PIFlevel, DAE) %>% 
  kbl("html", escape = FALSE, 
      caption = "Estimated Project Impacts on Key Indicators", 
      col.names = c("Indicator", "Raw Value", "Non-Burdened Areas Percentile", "Construction Phase Impact", "Operations Phase Impact", "Project Impact Rank", "Disproportionate Adverse Effect?")) %>% 
  kable_styling()
```
