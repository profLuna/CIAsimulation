...
@tbl-baseline shows statistics on frequency of exceedances using these different baselines. Values indicate the number of exceedances by census block group. For example, using the statewide baseline, the smallest number of exceedances for a burdened area block group is 4 exceedances, and maximum is 27. In comaparison, using the non-Burdened Area baseline, the smallest number of exceedances is 6, and the maximum is also 27. 
...
bind_cols(select(stateBM, Statistic, Statewide), select(nonBABM, nonBA)) %>% 
  mutate(Statewide = round(Statewide, 1),
         nonBA = round(nonBA, 1)) %>% 
  kbl("html", escape = FALSE, 
      caption = "Exceedances from Statewide versus non-Burdened Area Baselines", 
