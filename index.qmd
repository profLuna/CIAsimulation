...
read_xlsx("data/CIA Indicators by Technology 08-01-25.xlsx", 
  ...
  filter(!is.na(`Impact Category`) & `Impact Category` != "Site Suitability") %>% 
  select(-`Comments (Rationale for Inclusion)`) %>% 
  datatable(., rownames = FALSE, caption = "Key Indicators by Technology Type")
```
...
The project proponent will be provided with clear and concise guidance on assigning a project impact rank. Guidance should be provided for evaluating impacts on each unique indicator. 
...
data.frame(indicator = c(rep(">50",4), rep("40-49",4), rep("30-39",4), rep("<30",4)), 
           PIF = rep(c("Negligible", "Minor", "Moderate", "Major"),4),
           DAE = c("No", "Yes", "Yes", "Yes", "No", "No", "Yes", "Yes","No", "No", "No", "Yes", rep("No",4))) %>% 
  kbl("html", escape = FALSE, 
...
In the case of the current project simulation for the Sudbury Hudson transmission line, the project's projected impacts on key indicators show that it would not have a disproportionate adverse effect on any indicators.
...
BAscores %>% 
  ...
  mutate(DAE = case_when(
      PercentileNBA > 50 & PIFlevel %in% c("Minor Burdens", "Moderate Burdens", "Major Burdens") ~ "Yes",
      PercentileNBA >= 40 & PercentileNBA <= 49 & (PIFlevel == "Moderate Burdens" | PIFlevel == "Major Burdens") ~ "Yes",
      PercentileNBA >= 30 & PercentileNBA <= 39 & PIFlevel == "Major Burdens" ~ "Yes",
      .default = "No"
    )) %>% 
  select(indicator, RawValue, PercentileNBAcol, PIFlevel, DAE) %>% 
  kbl("html", escape = FALSE, 
      caption = "Estimated Project Impacts on Key Indicators", 
      col.names = c("Indicator", "Raw Value", "Percentile Relative to non-Burdened Areas of the State", "Project Impact Rank", "Disproportionate Adverse Effect?")) %>% 
  kable_styling()
```
